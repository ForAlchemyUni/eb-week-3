...
export function SheetMobileNav({ mainNavItems, sidebarNavItems }: SheetMobileProps) {
					...
					<div className="mt-2 mb-20">
						{mainNavItems?.length ? (
							<motion.div
								...
								animate="visible"
								initial="hidden"
								className="flex flex-col gap-8 text-lg text-foreground"
							>
						...
						{sidebarNavItems?.length ? (
							<div className="flex flex-col space-y-2 pt-4">
								{sidebarNavItems.map((item, index) => {
									const activeItems = item?.items?.filter((subItem) => !subItem.disabled);
